...
int turno(int matrix[FILAS][COLUMNAS], int jugador,int filaOrigen,int colOrigen,int filaDestino,int colDestino)//Gestiona movimientos y turnos
{
    ...
    if(jugador == FICHAS_BLANCAS || jugador == DAMA_BLANCA){//Comprueba que muevas solo donde haya fichas blancas  //***
        ...
        if(matrix[filaOrigen][colOrigen] != FICHAS_NEGRAS || DAMA_NEGRA){
    ...
    if(jugador == FICHAS_BLANCAS){//Comprueba que el movimiento de las BLANCAS no sea hacia atras
      ...
      //comprobar que no es dama porque si es dama se puede mover más de una posicion
        if(matrix[filaOrigen][colOrigen]== 4){
            //se puede mover las casillas que se quiera en diagonal

        }else if (matrix[filaOrigen][colOrigen]== 5){
            //se puede mover las casillas que se quiera en diagonal
        }else {
            ...
            //comprobar si una ficha llega al final se convierte en dama
            } else if (filaDestino == 8) {
                return 4;
            } else if (filaDestino == 1) {
                return 5;
            }
        }
